...
void merge(char *arq_1, char *arq_2, char *output) // função intercala
{
	FILE *a, *b, *saida;
	Endereco ea, eb;
  ...
  while (qntdArq >= 2) 
    ...
    proximo = 1;
		while (contaPares <= qntdArq)
		{
			...
			printf("%s intercalado com o proximo arquivo %s e se transformando em %s\n", arq_1, arq_2, output);
			merge(arq_1, arq_2, output); 
			remove(arq_1);					// deleta o arquivo que ja foi intercalado
			remove(arq_2);				// deleta o arquivo que ja foi intercalado
			proximo++;
			contaPares += 2; // passando de 2 em 2 pq estão sendo pegos 2 arquivos a cada while
		}
		qntdjuntos = qntdjuntos / 2; // a quantidade de blocos pares vai sendo dividida pela metade a cada passo do while
		qntdArq += qntdjuntos;
	}
	printf("\nTodos os %d blocos da parte inicial foram intercalados e transformados em um só arquivo chamado Arquivo_%d.dat", qntdArq, contaPares);


	printf("\nArquivo CEP_RJ%d.dat renomeado para CEP_RJ_INTEIRO_ORDENADO.dat\n", contaPares);
	sprintf(merged_name, "Arquivo_%d.dat", contaPares);
	rename(merged_name, "CEP_ordenado.dat"); // renomeia o arquivo intercalado para o CEP_ordenado.dat
}
